...
namespace USqlite
{
    public class BaseCommand
    {
        ...
        public virtual BaseReader Execute()
        {
            try
            {
                command = m_sqliteConnection.CreateCommand();
                command.CommandText = commandText;
                // UnityEngine.Debug.Log(string.Format("<color=green>USqlite execute : {0}</color>",command.CommandText));
                return new BaseReader(command);
            }
            catch (Exception @exception)
            {
                throw new USqliteException("Database Rollback" + @exception.InnerException.ToString());
            }
        }
        ...
        private string ConvertExpression2Sql<T>(Expression<Func<T, bool>> lambdaExpression)
        {
            ...
            // springdong 2020.1.6 :lambda express tostring result is And rather than AndAlso before .net4.0 , so replaced AndAlso to And in .net4.5
            sql = sql.Replace("AndAlso", "And");
        ...
        private void ConvertExpression2Sql(BinaryExpression binaryExpresion, ref Dictionary<string, string> keyValues)
        {
            ...
            else
            {
                ...
                else UnityEngine.Debug.Log("表达式转换失败");
