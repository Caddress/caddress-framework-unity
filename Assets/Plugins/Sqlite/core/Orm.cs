...
namespace USqlite
{
    public static class Orm
    {
        ...
        private static readonly Dictionary<Type,Func<BaseReader, int,object>> m_readFuncsDic = new Dictionary<Type,Func<BaseReader, int,object>>();
        private static readonly Dictionary<Type,Func<object,object>> m_writeFuncDic = new Dictionary<Type, Func<object, object>>();

        public static List<T> Mapping2List<T>( BaseReader dataReader )
        {
        ...
        public static Func<BaseReader,int,object> GetReadFunc(Type propertyFileType)
        {
            Func<BaseReader, int,object> func = null;
            if(!m_readFuncsDic.TryGetValue(propertyFileType,out func))
            {
                if(propertyFileType == typeof(Int32))
                    func = (reader,columnId) => reader.dataReader.GetInt32(columnId);
                else if(propertyFileType == typeof(Int64))
                    func = (reader,columnId) => reader.dataReader.GetInt64(columnId);
                else if(propertyFileType == typeof(string))
                    func = (reader,columnId) => reader.dataReader.GetString(columnId);
                else if(propertyFileType == typeof(bool))
                    func = (reader,columnId) => reader.dataReader.GetBoolean(columnId);
                else if(propertyFileType == typeof(float))
                    func = (reader,columnId) => reader.dataReader.GetFloat(columnId);
                else
                {
                    ...
                    {
                        var value = reader.dataReader.GetString(columnId);
