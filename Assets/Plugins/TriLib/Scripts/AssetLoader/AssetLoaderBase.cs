// ****************************************************************************
// 文件名称(File Name): AssetLoaderBase.cs
// 修改记录(Revision History):	
//  zhangjunyi  A2023_10_20    根据gltf文件中的alphaMode判断材质是否需要透明，解决材质使用了png贴图就会透明的问题
// ****************************************************************************
...
namespace TriLib
{
    ...
    public class AssetLoaderBase : IDisposable
    {
        ...
        protected void InternalLoadFromFile(string filename, string basePath, AssetLoaderOptions options = null, bool usesWrapperGameObject = false, AssimpInterop.ProgressCallback progressCallback = null)
        {
            ...
            {
                string text = string.Format("Error parsing file: {0}", filename);
                throw new Exception(text, exception);
            }
            if (Scene == IntPtr.Zero)
            {
                var error = AssimpInterop.ai_GetErrorString();
                string text = string.Format("Error loading asset. Assimp returns:\n [{0}]", error);
        ...
        protected virtual void TransformMaterialData(MaterialData materialData, AssetLoaderOptions options, string basePath = null)
        {
            ...
            //hzq 金属贴图用高光贴图
            metallicTexture = specularTexture == null ? metallicTexture : specularTexture;

            //A2023_10_20
            var alphaMode = materialData.GetPropertyByName("$mat.gltf.alphaMode") as MaterialProperty<string>;
            var checkAlphaChannel = alphaMode == null ? true : alphaMode.Data == "BLEND" ? true : false;

            var hasAlpha = checkAlphaChannel && hasAlphaChannelOnTextures || materialData.AlphaLoaded && materialData.Alpha < 1f;
            ...
            if ((options == null || options.ApplyDiffuseColor) && materialData.DiffuseColorLoaded)
            {
                ...
                {
                    color.a = materialData.Alpha == 0f ? 1 : materialData.Alpha;
                }
            ...
            {
                if(metallicTexture)
                Debug.Log($"metallic:  {metallicTexture} {metallicTexture.name}");
        ...
        private void BuildMeshes()
        {
            ...
            for (uint m = 0; m < meshCount; m++)
            {
                ...
                var hasTangentsAndBitangents = false;//AssimpInterop.aiMesh_HasTangentsAndBitangents(mesh);
        ...
        protected virtual void BuildMaterials(string basePath, AssetLoaderOptions options, LoadTextureDataCallback loadTextureDataCallback = null)
        {
            for (uint m = 0; m < AssimpInterop.aiScene_GetNumMaterials(Scene); m++)
            {
                ...
                var numNormals = AssimpInterop.aiMaterial_GetNumTextureHeight(material);
