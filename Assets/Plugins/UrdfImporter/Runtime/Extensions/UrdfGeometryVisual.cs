                ...
                try 
                {
                    ...
                    else if (meshFilePath.ToLower().EndsWith(".dae"))
                    {
                        ...
                        using (var assetLoader = new TriLib.AssetLoader())
                        {
                            ...
                            assetLoaderOptions.LoadRawMaterialProperties = false;//A_2023_10_20 开启读取材质，为后续判断材质是否需要透明做准备
                            meshObject = assetLoader.LoadFromFileWithTextures(meshFilePath, assetLoaderOptions);
                            Debug.Log("This Model Loaded success.");
                        }
                        if (meshObject != null) 
                        {
                    ...
                    //{
                    //    meshObject = OBJLoader.LoadOBJFile(meshFilePath);
                    //}
                }
                catch (Exception ex) 
                {
